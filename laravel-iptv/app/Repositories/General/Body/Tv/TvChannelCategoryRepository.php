...
class TvChannelCategoryRepository extends Controller implements ITvChannelCategoryRepository
{
    ...
    public function __construct(IUserHistoryLogRepository $logHistoriesRepo)
    {
        ...
        $this->activityName = 'tv channel category';
    }
    ...
    public function getData($request)
    {
        ...
        $query = $query->where(function($query) use ($search, $columns){
            foreach ($columns as $column) {
                if($column['searchable'] === "true"){    
                    $query->orWhere("tv_channel_categories.{$column['name']}", 'like', '%' . $search . '%');    
                }  
            }
        });
        ...
        $query->orderBy("tv_channel_categories.$orderByCol", $orderBy);
        ...
        foreach ($data as $value) {
            ...
            if(auth()->user()->canany(['tv_channel_categories.view','tv_channel_categories.update', 'tv_channel_categories.delete', 'tv_channel_categories.adb'])) {
                ...
                if(auth()->user()->can('tv_channel_categories.change_order')) {
                ...
                if(auth()->user()->can('tv_channel_categories.delete')) {
                    $actions .= '<li>
    ...
    {   
        try {
            ...
            $data = TvChannelCategory::find($post['id']);
    ...
    {   
        try {
            ...
            $data = TvChannelCategory::where('id', $id)->first();
