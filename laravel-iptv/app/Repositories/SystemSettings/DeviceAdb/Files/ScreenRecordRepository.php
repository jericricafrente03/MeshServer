...
class ScreenRecordRepository extends Controller implements IScreenRecordRepository
{
    use FileUploadTrait;
    ...
    public function __construct(IUserHistoryLogRepository $logHistoriesRepo)
    {
        ...
        $this->activityName = 'device adb files screen record';
    }
    ...
    public function getData($request)
    {
        ...
        $query = AdbFile::where('type', 'video');
        ...
        foreach ($data as $value) {
            ...
            if($value->img_uri){
                ...
                $img_uri = '<a href="' . asset($value->img_uri) . '" target="_blank"><video src="' . asset($value->img_uri) . '" autoplay controls muted class="media-object img img-thumbnail" style="width:350px"></video></a>';
            }
            else{
                $img_uri = '';
            }
    ...
    {
        $latestUpdate = AdbFile::where('type', 'video')->max('updated_at'); // Get latest update timestamp
