...
LOCKFILE="/tmp/adb_screen_record_$macAddress.lock"
...
# Echo datatime
echo "[$(date)] Screen Record Script started."
...
remoteDir="/sdcard/Gallery/ScreenRecords"
...
captureResult=$(adb -s $ipAddress:5555 shell "mkdir -p $remoteDir && screenrecord --time-limit 60 /sdcard/Gallery/ScreenRecords/${macAddress}${timestamp}.mp4 && echo 'Success'" 2>&1)
echo "Screen Record Result: $captureResult"
...
pullResult=$(adb -s $ipAddress:5555 pull $remoteDir/${macAddress}${timestamp}.mp4 $storeDir 2>&1)
...
imageDir="/storage/upload/device_adb/screen_record/${macAddress}${timestamp}.mp4"
insert_query="$sql \"INSERT INTO adb_files (ip4_address, mac_address, room, img_uri, type, created_at, updated_at) VALUES ('$ipAddress', '$macAddress', '$roomName', '$imageDir', 'video', NOW(), NOW());\""
eval $insert_query

# Log script completion
echo "[$(date)] Screen Record Script completed."