...
class FacilityRepository extends Controller implements IFacilityRepository
{
    use FileUploadTrait;
    ...
    public function getData($request)
    {
            ...
            foreach ($columns as $column) {
                if($column['searchable'] === "true"){    
                    if($column['name'] !== 'category') {
                        $query->orWhere("facilities.{$column['name']}", 'like', '%' . $search . '%');
                    }    
                }  
            }
            $query->orWhereHas('category', function ($query) use ($search) {
                $query->where("facility_categories.name", "like", '%' . $search . '%');
            });
        });

        $orderByCol = $columns[$orderColumnIndex]['name'];
        if ($orderByCol === 'category') {
            $query->join('facility_categories', 'facilities.category_id', '=', 'facility_categories.id')
                ->select('facilities.*', 'facility_categories.name as category_name')
                ->distinct()
                ->orderBy('facility_categories.name', $orderBy);
        }else {
            $query->orderBy("facilities.$orderByCol", $orderBy);
        }
        ...
        foreach ($data as $value) {
            ...
            if(auth()->user()->canany(['facilities.view','facilities.update', 'facilities.delete', 'facilities.enable'])) {
                ...
                if(auth()->user()->can('facilities.enable')) {
                ...
                if(auth()->user()->can('facilities.view')) {
                    $actions .= '<li><a class="dropdown-item" href="javascript:void(0)"  class="me-1"
    ...
    {   
        try {
            ...
            $customDirectory = 'upload/facilities';
