...
class FnbCategoryRepository extends Controller implements IFnbCategoryRepository
{
    ...
    public function __construct(IUserHistoryLogRepository $logHistoriesRepo)
    {
        ...
        $this->activityName = 'fnb category';
    }
    ...
    public function getData($request)
    {
        ...
        $query = $query->where(function($query) use ($search, $columns){
            foreach ($columns as $column) {
                if($column['searchable'] === "true"){    
                    $query->orWhere("fnb_categories.{$column['name']}", 'like', '%' . $search . '%');    
                }  
            }
        });
        ...
        $query->orderBy("fnb_categories.$orderByCol", $orderBy);
        ...
        foreach ($data as $value) {
            ...
            if(auth()->user()->canany(['fnb_categories.view','fnb_categories.update', 'fnb_categories.delete', 'fnb_categories.adb'])) {
                ...
                if(auth()->user()->can('fnb_categories.change_order')) {
                ...
                if(auth()->user()->can('fnb_categories.delete')) {
                    $actions .= '<li>
    ...
    {   
        try {
            ...
            $data = FnbCategory::where('id', $id)->first();
