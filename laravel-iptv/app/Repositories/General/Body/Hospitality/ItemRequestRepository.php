...
class ItemRequestRepository extends Controller implements IItemRequestRepository
{
    use FileUploadTrait;
    ...
    public function __construct(IUserHistoryLogRepository $logHistoriesRepo)
    {
        ...
        $this->activityName = 'item request';
    }
    ...
    public function getData($request)
    {
        ...
        $query = HospitalityItem::query()->whereNull('deleted_at');
        ...
        $query = $query->where(function($query) use ($search, $columns){
            foreach ($columns as $column) {
                if($column['searchable'] === "true"){
                    $query->orWhere("hospitality_items.{$column['name']}", 'like', '%' . $search . '%');   
                }  
            }
        });
        ...
        // Apply ordering
        $query->orderBy("hospitality_items.$orderByCol", $orderBy);
        ...
        foreach ($data as $value) {
            ...
            if(auth()->user()->canany(['item_requests.view','item_requests.update', 'item_requests.delete', 'item_requests.enable'])) {
                ...
                if(auth()->user()->can('item_requests.enable')) {
                ...
                if(auth()->user()->can('item_requests.delete')) {
                    $actions .= '<li>
    ...
    {   
        try {
            ...
            $customDirectory = 'upload/item_request';
    ...
    {
        try{
            $item = HospitalityItem::find($request->id);
    ...
    {   
        try {
            ...
            $data = HospitalityItem::find($post['id']);
    ...
    {   
        try {
            ...
            $data = HospitalityItem::where('id', $id)->first();
