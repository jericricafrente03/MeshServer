...
class RoomCategoryRepository extends Controller implements IRoomCategoryRepository
{
    ...
    public function __construct(IUserHistoryLogRepository $logHistoriesRepo)
    {
        ...
        $this->activityName = 'room category';
    }
    ...
    public function get_data($request)
    {
        ...
        $query = $query->where(function($query) use ($search, $columns){
            foreach ($columns as $column) {
                if($column['searchable'] === "true"){    
                    $query->orWhere("room_categories.{$column['name']}", 'like', '%' . $search . '%');    
                }  
            }
        });
        ...
        $query->orderBy("room_categories.$orderByCol", $orderBy);
        ...
        foreach ($data as $value) {
            ...
            if(auth()->user()->canany(['room_categories.view','room_categories.update', 'room_categories.delete', 'room_categories.adb'])) {
                ...
                if(auth()->user()->can('room_categories.change_order')) {
                ...
                if(auth()->user()->can('room_categories.delete')) {
                    $actions .= '<li>
    ...
    {   
        try {
            ...
            $data = RoomCategory::find($post['id']);
            ...
            if ($data->save()) {
                ...
                // $devices = Device::whereHas('room', function ($query) use ($data) {
